...
export default function AdminEvenements() {
  ...
  const supprimer = async (id, titre) => {
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1000 }}>
                <thead>
                  <tr style={{ borderBottom: `1px solid ${isDark ? '#2a2d3e' : '#e2e8f0'}` }}>
                    {['Événement', 'Type', 'Organisateur', 'Date', 'Capacité', 'Tickets', 'Statut', 'Actions'].map((h) => (
                      <th key={h} style={{ padding: '13px 16px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 1 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end' }} onClick={(e) => e.stopPropagation()}>
                          <ActionMenu 
                            options={[
                              ...
                              { divider: true },
                              { label: 'Supprimer', icon: 'bi-trash-fill', color: '#ef4444', onClick: () => supprimer(ev.id, ev.titre) }
                            ]}
                          />
                        </div>
                      </td>
